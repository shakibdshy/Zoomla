        ...
        <>
            <div className={`${profile ? 'block' : 'hidden'} border w-full bg-[#212534] pb-3 border-[#262938] rounded-2xl`}>
                ...
                <div className='px-4 flex justify-center'>
                    <Button onClick={() => setProfile(!profile)} size="md" className={`${profile ? "hover:bg-[#8e44ad] bg-[#8e44ad]" : ""} !text-white !text-center rounded-[10px] text-[16px] capitalize flex items-center`} variant="text">{profile ? 'My Profile' : "Feeds"}</Button>
                </div>
            </div>
            <Button onClick={() => setProfile(!profile)} size="md" className={`${profile ? "hidden" : "bg-blue-500 hover:bg-blue-500"} !text-white mx-auto !text-center rounded-[10px] text-[16px] capitalize flex items-center`} variant="text">{profile ? '' : "Feeds"}</Button>
        </>
    );
};
